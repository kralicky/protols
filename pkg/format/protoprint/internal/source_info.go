...
// code info.
type SourceInfoMap map[string][]*protoreflect.SourceLocation
...
func (m SourceInfoMap) Get(path []int32) *protoreflect.SourceLocation {
	v := m[asMapKey(path)]
	if len(v) > 0 {
		return v[0]
	}
...
func (m SourceInfoMap) GetAll(path []int32) []*protoreflect.SourceLocation {
	return m[asMapKey(path)]
}

// Add stores the given source code info for the given path.
func (m SourceInfoMap) Add(path []int32, loc *protoreflect.SourceLocation) {
...
func (m SourceInfoMap) PutIfAbsent(path []int32, loc *protoreflect.SourceLocation) bool {
	k := asMapKey(path)
	if _, ok := m[k]; ok {
		return false
	}
	m[k] = []*protoreflect.SourceLocation{loc}
	return true
}
...
func CreateSourceInfoMap(fd protoreflect.FileDescriptor) SourceInfoMap {
	res := SourceInfoMap{}
...
func PopulateSourceInfoMap(fd protoreflect.FileDescriptor, m SourceInfoMap) {
