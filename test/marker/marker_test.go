...
import (
	...
	"github.com/kralicky/protols/test/marker/expect"
	"github.com/kralicky/tools-lite/gopls/pkg/protocol"
	"github.com/kralicky/tools-lite/gopls/pkg/test/compare"
	...
	"github.com/kralicky/tools-lite/pkg/diff/myers"
...
func Test(t *testing.T) {
	...
	for _, test := range tests {
		...
		for file, contents := range test.files {
			edits := test.markerRemovalEdits[file]
			if len(edits) == 0 {
				continue
			}
			test.files[file], err = diff.ApplyBytes(contents, edits)
			if err != nil {
				panic(err)
			}
		}
		...
		t.Run(test.name, func(t *testing.T) {
			...
			run := &markerTestRun{
				test:       test,
				env:        newEnv(t, test.files, test.proxyFiles, test.writeGoSum, config),
...
func (mark marker) uri() protocol.DocumentURI {
	return mark.run.env.Sandbox.Workdir.URI(mark.note.Pos.Filename)
}
...
func (mark marker) path() string {
	return mark.run.env.Sandbox.Workdir.RelPath(mark.note.Pos.Filename)
}
...
func (mark marker) errorf(format string, args ...any) {
	...
	// subtest dividers.
	mark.T().Errorf("%s: %s", mark.note.Pos.String(), msg)
}
...
type markerTest struct {
	...
	markerRemovalEdits map[string][]diff.Edit        // edits which can be applied to files containing markers to remove them
...
func loadMarkerTest(name string, content []byte) (*markerTest, error) {
	...
	test := &markerTest{
		...
		markerRemovalEdits: make(map[string][]diff.Edit),
	...
	for _, file := range archive.Files {
		...
		default: // ordinary file content
			notes, err := expect.Parse(file.Name, file.Data)
			if err != nil {
				return nil, fmt.Errorf("parsing notes in %q: %v", file.Name, err)
			}

			// erase markers from the file content.
			var edits []diff.Edit
			comments := []ast.Comment{}
			for _, note := range notes {
				comments = append(comments, note.Comment)
			}
			comments = slices.Compact(comments)
			for _, comment := range comments {
				edits = append(edits, diff.Edit{
					Start: comment.Start().Offset,
					End:   comment.End().Offset + 1,
					New:   "",
				})
			}
			test.markerRemovalEdits[file.Name] = edits
...
func newEnv(t *testing.T, files, proxyFiles map[string][]byte, writeGoSum []string, config fake.EditorConfig) *integration.Env {
	...
	if err := awaiter.Await(ctx, integration.AllOf(
		integration.LogMatching(protocol.Info, "initialized workspace folders", 1, true),
	)); err != nil {
		sandbox.Close() // ignore error
		t.Fatal(err)
...
//   - ast.SourcePos: formatted using (*markerTestRun).fmtPos
...
// archive file.
func (run *markerTestRun) fmtPos(pos ast.SourcePos) string {
	m, err := run.env.Editor.Mapper(pos.Filename)
	if err != nil {
		run.env.T.Errorf("%s", err)
		return "<invalid location>"
	}
	loc, err := m.OffsetLocation(pos.Offset, pos.Offset)
	if err != nil {
		run.env.T.Errorf("Mapper(%s).PosLocation failed: %v", pos.Filename, err)
	}
...
func convertLocation(mark marker, arg any) (protocol.Location, error) {
	switch arg := arg.(type) {
	case string:
		startOff, preceding, m, err := linePreceding(mark.run, mark.note.Comment.Start())
		if err != nil {
	...
	case *regexp.Regexp:
		return findRegexpInLine(mark.run, mark.note.Comment.Start(), arg)
	default:
...
func findRegexpInLine(run *markerTestRun, pos ast.SourcePos, re *regexp.Regexp) (protocol.Location, error) {
...
func linePreceding(run *markerTestRun, endPos ast.SourcePos) (int, []byte, *protocol.Mapper, error) {
	m, err := run.env.Editor.Mapper(endPos.Filename)
	if err != nil {
		return 0, nil, nil, err
	}
	startOff, endOff, err := m.RangeOffsets(protocol.Range{
...
func snippetMarker(mark marker, src protocol.Location, item completionItem, want string) {
	...
	for _, i := range items {
		...
		if i.Label == item.Label {
			found = true
			if i.TextEdit != nil {
				switch v := i.TextEdit.Value.(type) {
...
func acceptCompletionMarker(mark marker, src protocol.Location, label string, golden *Golden) {
	...
	switch v := selected.TextEdit.Value.(type) {
	case protocol.InsertReplaceEdit:
		textEdits = []protocol.TextEdit{{Range: v.Replace, NewText: v.NewText}}
	case protocol.TextEdit:
		textEdits = []protocol.TextEdit{v}
	case nil:
		mark.errorf("expected CompletionItem.TextEdit to be non-nil")
		return
	}
	patched, _, err := protocol.ApplyEdits(mapper, append(textEdits, selected.AdditionalTextEdits...))
	if err != nil {
